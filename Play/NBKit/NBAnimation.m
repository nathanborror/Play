...
- (id) initWithKeyPath:(NSString*)keyPath {
	...
	if (self) {
		...
		self.stiffness = NBAnimationStiffnessMedium;
	}
	return self;
}
...
- (void) setStiffness:(NBAnimationStiffness)stiffness {
	[super setValue:@(stiffness) forKey:@"stifnessKey"];
	[self createValueArray];
}

- (NBAnimationStiffness) stiffness {
...
- (NSArray*) valueArrayForStartValue:(CGFloat)startValue endValue:(CGFloat)endValue {
	...
	if (self.stiffness == NBAnimationStiffnessHeavy) {
		stiffnessCoefficient = 0.001f;
	} else if (self.stiffness == NBAnimationStiffnessLight) {
